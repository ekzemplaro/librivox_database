...
{
	const key = key_number.toString(10)
...
{
	const file_json = "ex_" + key + '.json'
	const file_ft_json = "ft_" + key + '.json'

//	console.log (file_json)

	var https = require('https')

	const path_head='/api/feed/audiobooks/?id='
	const path_tail='&extended=1&format=json'

	const path_aa = path_head + key + path_tail
	var options = {
	host: 'librivox.org',
	port: 443,
	path: path_aa,
...
https.get(options, function(res) {
	...
	if (hantei != true)
		...
		fs.writeFile (file_ft_json,json_str_ft,function (err)
			{
			if (err) {
				console.error ("Error on write: " + err)
				}
...
//			console.log (data_ft[key].url_iarchive)
	...
	const json_str_ft = JSON.stringify (data_ft)
